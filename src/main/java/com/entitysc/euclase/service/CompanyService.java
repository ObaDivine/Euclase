 ...
 */
@Service
public class CompanyService extends EuclaseService {

    @Autowired
    MessageSource messageSource;
    @Value("${euclasews.api.company.list}")
    private String companyListUrl;
    @Value("${euclasews.api.company.create}")
    private String createCompanyUrl;
    @Value("${euclasews.api.company.update}")
    private String updateCompanyUrl;
    @Value("${euclasews.api.company.delete}")
    private String deleteCompanyUrl;
    @Value("${euclasews.api.company.fetch}")
    private String fetchCompanyUrl;

    public EuclaseResponsePayload processCreateCompany(EuclasePayload requestPayload) {
        try {
            ...
            requestPayload.setRequestType("Company");
            ...
            if (requestPayload.getId() == 0) {
                response = callEuclaseWSAPI(createCompanyUrl, gson.toJson(requestPayload), generateEuclaseWSAPIToken(), "Company");
            } else {
                response = callEuclaseWSAPI(updateCompanyUrl, gson.toJson(requestPayload), generateEuclaseWSAPIToken(), "Company");
            }
            //Check for error
    ...
    public EuclaseResponsePayload processDeleteCompany(String id, String principal) {
        try {
            ...
            String response = callEuclaseWSAPI(deleteCompanyUrl + "?id=" + encodedParam + "&prcp=" + encodedPrincipal, "GET", generateEuclaseWSAPIToken(), "Company Delete");
            //Check for error
            if (response.contains("error")) {
                ExceptionPayload responsePayload = gson.fromJson(response, ExceptionPayload.class);
                EuclaseResponsePayload exceptionResponse = new EuclaseResponsePayload();
    ...
    public EuclaseResponsePayload fetchCompany(String id) {
        try {
            String encodedParam = urlEncodeString(encryptString(id.trim()));
            String response = callEuclaseWSAPI(fetchCompanyUrl + "?id=" + encodedParam, "GET", generateEuclaseWSAPIToken(), "Company Fetch");
            //Check for error
            if (response.contains("error")) {
                ExceptionPayload responsePayload = gson.fromJson(response, ExceptionPayload.class);
                EuclaseResponsePayload exceptionResponse = new EuclaseResponsePayload();
    ...
    public DataListResponsePayload fetchCompanyList() {
        try {
            EuclasePayload euclasewsPayload = new EuclasePayload();
            euclasewsPayload.setChannel("WEB");
            euclasewsPayload.setRequestId(generateRequestId());
            euclasewsPayload.setToken(generateEuclaseWSAPIToken());
            //Connect to EuclaseWS API
            String response = callEuclaseWSAPI(companyListUrl, "GET", generateEuclaseWSAPIToken(), "Company List");
