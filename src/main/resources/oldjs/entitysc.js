(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.entitysc = factory(global.bootstrap));
  ...
  const getColor = (name, dom = document.documentElement) => {
    return getComputedStyle(dom).getPropertyValue(`--entitysc-${name}`).trim();
  };
  ...
  const bigPictureInit = () => {
    const { getData } = window.entitysc.utils;
  ...
  const basicEchartsInit = () => {
    const { getColor, getData, getDates } = window.entitysc.utils;
  ...
  const detectorInit = () => {
    const { addClass } = window.entitysc.utils;
    const { is } = window;
  ...
  function initMap() {
    ...
    if ($googlemaps.length && window.google) {
      ...
      $googlemaps.forEach(itm => {
        ...
        const mapStyle = getData(itm, 'entityscTheme');

        if (getData(itm, 'entityscTheme') === 'streetview') {
        ...
        const mapOptions = {
          ...
          styles:
            window.config.config.entityscTheme === 'dark'
        ...
        lists.forEach(el => {
          ...
          if (bulkSelect) {
            const bulkSelectInstance =
              window.entitysc.BulkSelect.getInstance(bulkSelect);
  ...
  const modalInit = () => {
    const $modals = document.querySelectorAll('[data-entitysc-modal]');
  ...
  const handleNavbarVerticalCollapsed = () => {
    const { getItemFromStore, hasClass, setItemToStore } = window.entitysc.utils;
    const Selector = {
      HTML: 'html',
    ...
    if (navbarVerticalToggle) {
      navbarVerticalToggle.addEventListener(Events.CLICK, e => {
        ...
        const isNavbarVerticalCollapsed = getItemFromStore(
          'entityscIsNavbarVerticalCollapsed',
          false
        );
        setItemToStore(
          'entityscIsNavbarVerticalCollapsed',
  ...
  const entityscOffcanvasInit = () => {
    const { getData } = window.entitysc.utils;
    const toggleEls = document.querySelectorAll(
      "[data-entitysc-toggle='offcanvas']"
    );
    const offcanvasBackdrop = document.querySelector('[data-entitysc-backdrop]');
  ...
  const productDetailsInit = () => {
    const { getData, resize } = window.entitysc.utils;
  ...
  const ratingInit = () => {
    const { getData, getItemFromStore } = window.entitysc.utils;
    const raters = document.querySelectorAll('[data-rater]');

    raters.forEach(rater => {
      const options = {
        reverse: getItemFromStore('entityscIsRTL'),
        starSize: 32,
        step: 0.5,
        element: rater,
  ...
  const responsiveNavItemsInit = () => {
    const { resize } = window.entitysc.utils;
    const Selector = {
  ...
  const initialDomSetup = element => {
    ...
    element.querySelectorAll('[data-theme-control]').forEach(el => {
      ...
      if (el.type === 'checkbox') {
        if (inputDataAttributeValue === 'entityscTheme') {
        ...
        el.type === 'radio' &&
        inputDataAttributeValue === 'entityscNavbarVerticalStyle'
      ) {
        ...
        el.type === 'radio' &&
        inputDataAttributeValue === 'entityscNavbarTopShape'
      ) {
        ...
        el.type === 'radio' &&
        inputDataAttributeValue === 'entityscNavbarTopStyle'
      ) {
        localStorageValue === 'darker' &&
          el.value === 'darker' &&
          el.setAttribute('checked', true);
  ...
  const changeTheme = element => {
    ...
    element
      .querySelectorAll('[data-theme-control = "entityscTheme"]')
      .forEach(el => {
        const inputDataAttributeValue = getData(el, 'theme-control');
        const localStorageValue = getItemFromStore(inputDataAttributeValue);
    ...
    themeController.on('click', e => {
      ...
      if (target.data('theme-control')) {
        ...
        switch (control) {
          ...
          case 'entityscNavbarVerticalStyle': {
          ...
          case 'entityscNavbarTopShape':
            {
              handlePageUrl(target.node);
            }
            break;
          case 'entityscNavbarPosition':
            {
              handlePageUrl(target.node);
            }
          ...
          case 'entityscIsRTL':
            {
              // localStorage.setItem('entityscIsRTL', target.node.checked);
              window.config.set({
                entityscIsRTL: target.node.checked
              });
              window.location.reload();
            }
            break;
  ...
  const tinymceInit = () => {
    const { getColor, getData, getItemFromStore } = window.entitysc.utils;
    ...
    if (window.tinymce) {
      tinymces.forEach(tinymceEl => {
        ...
        `,
            ...
            directionality: getItemFromStore('entityscIsRTL') ? 'rtl' : 'ltr',
            toolbar: [
  ...
  docReady(entityscOffcanvasInit);
  ...
  docReady(() => {
    ...
    if (selectedRowsBtn) {
      ...
      const bulkSelectInstance =
        window.entitysc.BulkSelect.getInstance(bulkSelectEl);
...
//# sourceMappingURL=entitysc.js.map
